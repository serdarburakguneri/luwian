...
/**
 * JSON logging helper:
 * - If logstash-logback-encoder is on classpath and luwian.logging.json=true,
 *   log a startup hint so users know JSON logging is active or explain how to enable it.
 *
 * We intentionally do not force a logging config because Spring resolves logging setup
 * before auto-configuration. Users keep full control of logback-spring.xml.
 */
public class LuwianJsonLoggingConfig {
    ...
    // Class names
    private static final String LOGSTASH_ENCODER_CLASS = "net.logstash.logback.encoder.LogstashEncoder";

    // Log messages
    private static final String MSG_JSON_SUPPORTED = "Luwian: JSON logging is supported (logstash encoder detected). " +
            "Ensure your logback-spring.xml uses LogstashEncoder for JSON output.";
    private static final String MSG_JSON_MISSING = "Luwian: JSON logging requested, but Logstash encoder not found. " +
